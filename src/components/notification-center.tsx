...
export type NotificationItem = {
  ...
  title: string;
  message: string;
  // The real value is one of notification-service.ts's event names
  // (e.g. "NEW_BOOKING", "PAYMENT_SUCCESS") — never actually a UI
  // category. Use getNotificationCategory() to map it to one.
  type: string;
  read: boolean;
  createdAt: string;
  link?: string | null;
...
export type NotificationCategory = "SUCCESS" | "WARNING" | "SYSTEM" | "INFO";

// Maps every real event name notifyUser() is ever called with (see
// notification-service.ts) to the UI category that drives its icon/color.
// Anything not listed here (a future event type someone forgets to add)
// safely falls back to INFO rather than rendering blank.
const NOTIFICATION_CATEGORY: Record<string, NotificationCategory> = {
  ...
  OFFER_REJECTED: "WARNING",
  COURSE_PUBLISHED: "SUCCESS",
  COURSE_PURCHASED: "SUCCESS",
  COURSE_UNLOCKED: "SUCCESS",
  PASSWORD_CHANGED: "SYSTEM",
  PROFESSOR_VERIFIED: "SUCCESS",
  PROFESSOR_REJECTED: "WARNING",
