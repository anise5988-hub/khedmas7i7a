...
export async function sendOtpEmail(to: string, name: string | undefined, otp: string, title: string, message: string): Promise<boolean> {
  const apiKey = process.env.BREVO_API_KEY;
  if (!apiKey || !senderEmail || !to) {
    console.warn("OTP email skipped: BREVO_API_KEY/PROFY_EMAIL_FROM is not configured.");
    return false;
  }
                ...
                <tr>
                  <td style="padding:32px;">
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" style="margin:28px 0 12px 0;">
                      <tr>
                        <td style="border-radius:16px;background-color:#0d8d78;">
                          <div style="display:inline-block;padding:14px 28px;font-size:24px;font-weight:bold;color:#ffffff;text-decoration:none;border-radius:16px;letter-spacing:4px;">
                            ${escapeHtml(otp)}
                          </div>
                        </td>
                      </tr>
  ...
  try {
    const response = await fetch("https://api.brevo.com/v3/smtp/email", {
      method: "POST",
      headers: { "api-key": apiKey, "Content-Type": "application/json", Accept: "application/json" },
      body: JSON.stringify({ sender: { name: "PROFY", email: senderEmail }, to: [{ email: to, name }], subject: title, htmlContent: otpHtml }),
    });
    if (!response.ok) console.error("Brevo OTP email failed", response.status, await response.text());
    return response.ok;
  } catch (error) {
