 ...
 * /teachers directory, and the AI matcher) reads from this single query so
 * they never drift out of sync with each other or with the DB-vs-fallback
 * failover behavior.
 */
export async function getApprovedTeachers(): Promise<DirectoryTeacher[]> {
  try {
